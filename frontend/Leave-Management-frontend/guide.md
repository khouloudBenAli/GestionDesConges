# Guide de lancement des services backend

Ce projet est composé de 4 services Spring Boot (Eureka, Employee, Leave, API Gateway),
d'une infrastructure Docker (Postgres x2, Kafka, Zookeeper, Kafka UI) et d'un frontend React (Vite).

## Prérequis

- Java 17
- Maven (`mvn -v`)
- Docker Desktop (doit être démarré)

## Étape 1 — Démarrer l'infrastructure (Postgres + Kafka)

Depuis la racine du projet (`leave-management-microservices/`) :

D:\iTeam\thirdYear\PFA\backend\leave-management-microservices

```bash
docker compose up -d
```

Cela démarre 5 conteneurs :

| Conteneur | Rôle | Port |
|---|---|---|
| `employee-db` | PostgreSQL — base `employee_db` | 5432 |
| `leave-db` | PostgreSQL — base `leave_db` | 5433 |
| `zookeeper` | requis par Kafka | 2181 |
| `kafka` | broker Kafka | 9092 |
| `kafka-ui` | interface web pour Kafka | 8090 |

Vérifier que tout est démarré :

```bash
docker compose ps
```

Attendre que `employee-db`, `leave-db` et `kafka` soient `healthy` avant de lancer les services Java
(sinon ils échoueront à se connecter).

## Étape 2 — Démarrer les services Spring Boot (dans cet ordre)

Chaque service doit être lancé dans son propre terminal, **dans l'ordre suivant**, car chacun
s'enregistre auprès d'Eureka et le gateway route vers les autres services via Eureka.

### 2.1 Eureka Server (registre de services)

```bash
cd eureka-server

D:\iTeam\thirdYear\PFA\backend\leave-management-microservices\eureka-server

mvn spring-boot:run
```

Attendre que `http://localhost:8761` réponde (dashboard Eureka) avant de continuer.

### 2.2 Employee Service

```bash
cd employee-service

D:\iTeam\thirdYear\PFA\backend\leave-management-microservices\employee-service

mvn spring-boot:run
```

- Port : `8081`
- Base de données : `employee_db` (port 5432)

### 2.3 Leave Service

```bash
cd leave-service


D:\iTeam\thirdYear\PFA\backend\leave-management-microservices\leave-service

mvn spring-boot:run
```

- Port : `8082`
- Base de données : `leave_db` (port 5433)

(2.2 et 2.3 peuvent être démarrés en parallèle.)

### 2.4 API Gateway

```bash
cd api-gateway

D:\iTeam\thirdYear\PFA\backend\leave-management-microservices\api-gateway

mvn spring-boot:run
```

- Port : `8085`
- Route `/api/auth/**` (login/register/me/logout) directement
- Route `/api/employees/**`, `/api/departments/**` → `employee-service` via Eureka
- Route `/api/leave-types/**`, `/api/leave-requests/**`, `/api/leave-balances/**` → `leave-service` via Eureka

À démarrer **en dernier**, une fois que employee-service et leave-service sont enregistrés dans Eureka.

## Étape 3 — Démarrer le frontend

```bash
cd Leave-Management-frontend
D:\iTeam\thirdYear\PFA\frontend\Leave-Management-frontend
npm run dev
```

- Port : `3000`
- Le proxy Vite (`vite.config.ts`) redirige `/api/*` vers les services backend correspondants.

## Vérifications rapides

| URL | Attendu |
|---|---|
| `http://localhost:8761` | Dashboard Eureka listant `EMPLOYEE-SERVICE`, `LEAVE-SERVICE`, `API-GATEWAY` |
| `http://localhost:8085/actuator/health` | `200` (ou `401` si authentification requise — signe que le service tourne) |
| `http://localhost:8090` | Kafka UI |
| `http://localhost:3000` | Page de connexion |

## Comptes par défaut

| Utilisateur | Mot de passe | Rôle |
|---|---|---|
| `admin` | `admin123` | ADMIN |
| `manager` | `manager123` | MANAGER |
| `employee` | `employee123` | EMPLOYEE |

## Visualiser la base de données

Les deux bases tournent dans des conteneurs Postgres (`employee-db` sur le port 5432,
`leave-db` sur le port 5433, utilisateur `postgres` / mot de passe `postgres`).

### Option A — `psql` directement dans le conteneur (rapide, sans installation)

```bash
# Base employee_db
docker exec -it employee-db psql -U postgres -d employee_db

# Base leave_db
docker exec -it leave-db psql -U postgres -d leave_db
```

Une fois dans `psql` :

```sql
-- Lister les tables
\dt

-- Voir la structure d'une table
\d employees

-- Quitter
\q
```

Requêtes utiles une fois connecté à **employee_db** :

```sql
SELECT * FROM departments;

SELECT id, first_name, last_name, email, job_title, department_id
FROM employees;

SELECT e.first_name, e.last_name, d.name AS department
FROM employees e
LEFT JOIN departments d ON d.id = e.department_id;
```

Requêtes utiles une fois connecté à **leave_db** :

```sql
SELECT * FROM leave_types;

SELECT id, employee_id, leave_type_id, start_date, end_date, status, number_of_days
FROM leave_requests
ORDER BY created_at DESC;

SELECT employee_id, leave_type_id, remaining_days
FROM leave_balances;

-- Demandes en attente
SELECT * FROM leave_requests WHERE status = 'PENDING';
```

### Option B — Client graphique (pgAdmin, DBeaver, TablePlus, extension VS Code "PostgreSQL")

Paramètres de connexion :

| Paramètre | employee_db | leave_db |
|---|---|---|
| Host | `localhost` | `localhost` |
| Port | `5432` | `5433` |
| Database | `employee_db` | `leave_db` |
| User | `postgres` | `postgres` |
| Password | `postgres` | `postgres` |

Dans VS Code, l'extension "PostgreSQL" (ou "SQLTools" + driver PostgreSQL) permet de parcourir
les tables et lancer des requêtes directement depuis l'éditeur avec ces mêmes paramètres.

### Visualiser les événements Kafka

Pour voir les événements publiés entre les services (ex: `employee-created`, `leave-status-changed`) :

```
http://localhost:8090
```

(Kafka UI, déjà démarré avec `docker compose up -d` — sélectionner le cluster `local` puis l'onglet `Topics`.)

## Problèmes fréquents

- **502 / ECONNREFUSED sur le frontend** : le service backend ciblé par le proxy n'est pas démarré (vérifier l'ordre de démarrage).
- **"Port 8085 already in use"** : un ancien processus `java` tourne encore (le wrapper `mvn` a été arrêté mais pas le processus Java enfant). Trouver le PID avec `Get-NetTCPConnection -LocalPort 8085` puis `Stop-Process -Id <PID> -Force`.
- **Création d'employé très lente (~60s) ou en erreur 500** : Kafka n'est pas démarré ou pas encore prêt — vérifier `docker compose ps` et que `kafka` est `healthy`.


-------------------------------------------------

Arrêter le projet proprement
1. Frontend + Services Spring Boot
Dans chaque terminal où tu as lancé un service, appuie sur :


Ctrl + C
Faire ça pour les 5 terminaux (frontend, eureka, employee-service, leave-service, api-gateway).

Attention : parfois sur Windows, Ctrl+C arrête Maven mais laisse le processus Java tourner en arrière-plan. Si tu vois une erreur "Port already in use" au prochain démarrage, tu dois tuer le processus Java manuellement :


# Trouver quel processus utilise le port (ex: 8085)
Get-NetTCPConnection -LocalPort 8085 | Select-Object OwningProcess
# Puis tuer ce processus
Stop-Process -Id <PID> -Force
2. Docker (Kafka + PostgreSQL)

docker compose down
Cette commande arrête et supprime les conteneurs. Les données en base sont conservées (dans des volumes Docker).

Si tu veux aussi supprimer les données (repartir à zéro) :


docker compose down -v
-v supprime les volumes — les bases de données seront vides au prochain démarrage. À éviter sauf si tu veux vraiment tout réinitialiser.

Ordre recommandé pour fermer
Ctrl+C dans le terminal du frontend
Ctrl+C dans le terminal de l'API Gateway
Ctrl+C dans les terminaux employee-service et leave-service
Ctrl+C dans le terminal Eureka
docker compose down dans le terminal à la racine du projet backend
