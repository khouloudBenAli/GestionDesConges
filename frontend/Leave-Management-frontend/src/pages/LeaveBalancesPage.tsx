...
import { enrichBalances } from '../utils/enrich'
...
export default function LeaveBalancesPage() {
    ...
    Promise.all([
      ...
      .then(([emps, types]) => {
        ...
        if (isEmployee && user?.employeeId) {
          ...
          loadBalances(String(user.employeeId), emps.data, types.data)
        }
      })
  ...
  const loadBalances = async (empId: string, emps: Employee[] = employees, types: LeaveType[] = leaveTypes) => {
    if (!empId) return
    setLoading(true)
    try {
      const { data } = await leaveBalanceApi.getByEmployee(Number(empId))
      const raw = Array.isArray(data) ? data : [data]
      setBalances(enrichBalances(raw, emps, types))
    }
        ...
        {!faded && (
          <>
            <div className="flex items-center gap-2 mb-3">
              {(b.leaveType?.colorCode ?? b.leaveTypeColor) && <span className="w-3 h-3 rounded-full" style={{ backgroundColor: b.leaveType?.colorCode ?? b.leaveTypeColor }} />}
              <h3 className="font-semibold text-gray-900">{b.leaveType?.name ?? b.leaveTypeName ?? 'Congé'}</h3>
            </div>
        ...
        {faded && (
          <>
            <div className="flex items-center justify-between mb-2">
              <h3 className="font-medium text-gray-700">{b.leaveType?.name ?? b.leaveTypeName ?? 'Congé'}</h3>
