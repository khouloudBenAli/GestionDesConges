...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    Promise.allSettled(fetches).then(([depts, emps, reqs, types]) => {
      ...
      const enriched = enrichRequests(requests as LeaveRequest[], employees as Employee[], leaveTypes as LeaveType[])
      setRecentRequests(
        [...enriched].sort((a, b) => new Date(b.createdAt ?? 0).getTime() - new Date(a.createdAt ?? 0).getTime()).slice(0, 8)
      )
      setLoading(false)
    })
  }, [isAdmin, isManager])
